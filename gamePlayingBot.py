...
# gamePlayingBot - Play the game before it plays YOU, #MyGuy
...
logging.basicConfig(level=logging.DEBUG,
                    formate='%(asctime)s.%(msecs)03d: %(message)s', datefmt='%H:%M:%S')
# loggingdisable(logging.DEBUG)     # uncomment to block debug log messages

'''
This generic code sets up a shebang line, some comments to describe the program, imports for
several modules, and setting up logging so that the logging.debug() function will output debug
messages. (I explain why logging is preferable to print() calls in this blog post.
...
'''
Next, there are several variable constants set up for this program. I use constants as a way to
detect typos; mistyping a string value as 'califrnia_roll' will cause bugs but Python won't directly
point out the typo. Mistyping a variable name as CALIFRNIA_ROLL will cause Python to raise a
NameError exception, letting you quickly fix it.
'''
...
ALL_ORDER_TYPES = (ONIGIRI, GUNKAN_MAKI, CALIFORNIA_ROLL,
                   SALMON_ROLL, SHRIMP_SUSHI, UNAGI_ROLL, DRAGON_ROLL, COMBO)

'''
Next, set up constants for each of the ingredients and the recipes for each order.
(This tutorial saves you from having to look up the recipe information in the game yourself.)
'''
...
'''
Next are several constants and global variables used in the program. Although
using global variables is generally a bad idea for making maintainable programs,
this Python script is just a one off and I didn't want to make it complicated for newer
programmers who don't yet know object-oriented programming concepts.
...
# the number of seconds it takes to restock inventory after ordering it (at normal speed, not express)
NORMAL_RESTOCK_TIME = 7
# if an order goes unfilled for this number of seconds, remake it
TIME_TO_REMAKE = 30

# Global variables
LEVEL = 1                   # current level being played
INVENTORY = {SHRIMP: 5, RICE: 10, NORI: 10, ROE: 10, SALMON: 5, UNAGI: 5}
# (left, top, width, height) values coordinates of the game window
GAME_REGION = ()
# unix timestamp when an ordered ingredient will have arrived
ORDERING_COMPLETE = {SHRIMP: None, RICE: None,
                     NORI: None, ROE: None, SALMON: None, UNAGI: None}
# unix timestamp of when the rolling of the mat will have completed
ROLLING_COMPLETE = 0
LAST_PLATE_CLEARING = 0     # unix timestamp of the last time the plates were cleared
# unix timestamp when we last checked for the Game Over or You Win messages
LAST_GAME_OVER_CHECK = 0

'''
Note: a "unix timestamp" is the number of seconds since midnight, January 1, 1970. It is
returned by the time.time() function. For example, if time.time() is called at 10:36:54.392
PM on December 16, 2014, it will return the value 1418798214.392. Ten seconds later it will
return the value 1418798224.392. Subtracting these values is how the bot can measure how
much time has passed.

And finally set up some global variables which will store coordinates for various things
in the game window. The GAME_REGION variable stores the position of the Sushi Go Round
game itself. Once that value has been acquired, the setupCoordinates() function
(described later) will populate the values in these variables:
'''
...
def main():
    ...
    logging.debug(
        'To interrupt mouse movement, move mouse to upper left corner.')
    getGameRegion()
...
def getGameRegion():
    ...
    GAME_REGION = (topRightX - 640, topRightY, 640, 480)
...
def setupCoordinates():
    ...
    NORMAL_DELIVERY_BUTTON_COORDS = (
        GAME_REGION[0] + 495, GAME_REGION[1] + 293)
...
def getOrders():
    ...
    for orderType in (ALL_ORDER_TYPES):
        allOrders = pyautogui.locateAllOnScreen(imPath('%s_order.png' % orderType), region=(
            GAME_REGION[0] + 32, GAME_REGION[1] + 46, 558, 44))
...
for ingredient, amount in RECIPE[orderType].items():
    if INVENTORY[ingredient] < amount:
        logging.debug('More %s is needed to make %s.' %
...
# get rid of any left over meals on the conveyor belt, which may stall this meal from being loaded on the belt
...
def findAndClickPlatesOnBelt():
    ...
    for color in ('pink', 'blue', 'red'):
        result = pyautogui.locateCenterOnScreen(imPath('%s_plate_color.png' % (
            color)), region=(GAME_REGION[0] + 343, GAME_REGION[1] + 300, 50, 80))
        if result is not None:
            pyautogui.click(result)
            logging.debug('Clicked on %s plate on belt at X: %s Y: %s' %
...
def orderIngredient(ingredient):
    ...
    logging.debug('Ordering more %s (inventory says %s left)...' %
    ...
    if ingredient == RICE and ORDERING_COMPLETE[RICE] is None:
        ...
        if pyautogui.locateOnScreen(imPath('cant_afford_rice.png'), region=(GAME_REGION[0] + 498, GAME_REGION[1] + 242, 90, 75)):
            ...
            pyautogui.click(GAME_REGION[0] + 585,
                            GAME_REGION[1] + 335, duration=0.25)
            return
        ...
        if pyautogui.locateOnScreen(imPath('cant_afford_%s.png' % (ingredient)), region=(GAME_REGION[0] + 446, GAME_REGION[1] + 187, 180, 180)):
            ...
            # click cancel phone button
            pyautogui.click(GAME_REGION[0] + 597,
                            GAME_REGION[1] + 337, duration=0.25)
            return
...
def checkForGameOver():
    ...
    result = pyautogui.locateOnScreen(imPath('you_win.png'), region=(
        GAME_REGION[0] + 188, GAME_REGION[1] + 94, 262, 60))
    if result is not None:
        pyautogui.click(pyautogui.center(result))
        return LEVEL_WIN_MESSAGE

    # check for "You Fail" message
    result = pyautogui.locateOnScreen(imPath('you_failed.png'), region=(
        GAME_REGION[0] + 167, GAME_REGION[1] + 133, 314, 39))
    if result is not None:
